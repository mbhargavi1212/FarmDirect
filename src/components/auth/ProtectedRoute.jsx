...
export const ProtectedRoute = ({ children, allowedRoles, onNavigate, portalName, }) => {
    const { user, isAuthenticated, role, logout } = useAuth();
    // 1. If not authenticated, prompt user to go to FarmDirect login
    if (!isAuthenticated || !user) {
        return (<div className="min-h-[75vh] flex items-center justify-center p-4 sm:p-6 bg-slate-50">
        <div className="max-w-md w-full bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 text-center shadow-md space-y-5">
          ...
          <div className="flex flex-col gap-2.5 pt-1">
            <Button variant="primary" className="w-full text-sm py-2.5 font-semibold" icon={LogIn} onClick={() => onNavigate('/login')}>
              Go to FarmDirect Login
            </Button>
            <Button variant="outline" className="w-full text-sm py-2.5" icon={UserPlus} onClick={() => onNavigate('/register')}>
    ...
    if (allowedRoles && !allowedRoles.includes(role)) {
        ...
        <div className="max-w-md w-full bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 text-center shadow-md space-y-5">
          ...
          <div className="flex flex-col gap-2.5 pt-1">
            <Button variant="primary" className="w-full text-sm py-2.5" icon={ArrowRight} onClick={() => onNavigate(`/${role}`)}>
              Return to My {role === 'farmer' ? 'Farmer' : role === 'buyer' ? 'Consumer' : 'Admin'} Portal
            </Button>
            <Button variant="outline" className="w-full text-sm py-2.5" icon={LogOut} onClick={() => {
                logout();
                onNavigate('/login');
            }}>
